...
class BrachaRB(BasicDolevRC):
    ...
    async def on_send(self, peer: Peer, payload: SendMessage):
        ...
        self.msg_log.log(
            LOG_LEVEL.DEBUG, f"Received a SEND message: {payload.message_id}."
        )
        ...
        await self.trigger_send_echo(message_id, payload)
    ...
    async def on_echo(self, peer: Peer, payload: EchoMessage):
        ...
        self.msg_log.log(
            LOG_LEVEL.DEBUG, f"Received an ECHO message: {payload.message_id}."
        )
    ...
    async def on_ready(self, peer: Peer, payload: ReadyMessage):
        ...
        self.msg_log.log(
            LOG_LEVEL.DEBUG, f"Received a READY message: {payload.message_id}."
        )
